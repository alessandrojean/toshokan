<template>
  <div class="select-none rounded border border-gray-300 dark:border-gray-600 divide-y divide-gray-300 dark:divide-gray-600 flex flex-col">
    <div
      v-for="(step, i) in filteredSteps"
      :key="i"
      :class="[
        'group px-4 py-2 flex items-center space-x-4 disabled:cursor-default',
        modelValue === i + 1 ? 'text-primary-600 dark:text-primary-400' : 'text-gray-500 dark:text-gray-400'
      ]"
    >
      <span
        :class="[
          'text-xs font-semibold leading-none flex items-center justify-center w-8 h-8 border-2 border-gray-300 dark:border-gray-600 rounded-full',
      ...
      <span
        :class="['font-medium text-sm', modelValue > i + 1 ? 'text-gray-800 dark:text-gray-200' : '']"
      >
        {{ step.title }}
      </span>
    </div>
